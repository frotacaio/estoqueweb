...
namespace EstoqueWeb.Controllers
{
    public class EnderecoController : Controller 
    {
        ...
        public async Task<IActionResult> Index(int? cid)//Retorna listagem de Enderecos ordenando pelo nome
        {
            if(cid.HasValue)
            {
        ...
        private bool EnderecoExiste(int? cid, int? eid) //Método para verificar se o id existe no banco
        {
        ...
        {
            if(!cid.HasValue) // se id do cliente não tiver valor
            {
                TempData["mensagem"] = MensagemModel.Serializar("Cliente não informado",TipoMensagem.Erro);
                return RedirectToAction("Index","Cliente");
            }
            if(!eid.HasValue) // se id do endereço não tiver valor
            {
                TempData["mensagem"] = MensagemModel.Serializar("Cliente não informado",TipoMensagem.Erro);
                return RedirectToAction("Index",new {cid = cid});
            }
            var cliente = await _context.Clientes.FindAsync(cid); //Buscando cliente pelo Id
            var endereco = cliente.Enderecos.FirstOrDefault(e => e.IdEndereco == eid); //verificando qual endereço pertence aquele cliente
            if(endereco == null) // Se não for encontrado endeço relacionado ao cliente...
            {
                TempData["mensagem"] = MensagemModel.Serializar("Endereço não encontrado.",TipoMensagem.Erro);
                return RedirectToAction("Index", new {cid = cid});
            }
            ViewBag.Cliente = cliente; // Guarda cliente na ViewBag
            return View(endereco); // Retorna a view com o endereço localizado
        ...
        public async Task<IActionResult> Excluir(int idUsuario, int idEndereco)
        {
            ...
            if(endereco != null)
            {
                ...
                if(await _context.SaveChangesAsync() > 0)
                {
                    TempData["mensagem"] = MensagemModel.Serializar("Endereco Excluído com sucesso");
                ...
                {
                    TempData["mensagem"] = MensagemModel.Serializar("Não foi possível excluir o endereco",TipoMensagem.Erro);
                }
            }
            else
            {
